...
predictor = dlib.shape_predictor("H:/Python/shape_predictor_68_face_landmarks.dat")
...
while True:
	...
	#grab the frames from the threaded video file stream, resize it, and convert it to grayscale channels
	frame = vs.read()
	frame = imutils.resize(frame, width=450)
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

	#detect faces in the frayscale frame
	...
	for rect in rects:
		...
		#average the eye aspect ration together for both eyes
		...
		#otherwise, the eye aspect ratio is not below the blink threshold
			...
			#if the the eyes were closed for a sufficient number of then incrfeament the total number of blinks
			if CONNTER >= EYE_AR_CONSEC_FRAMES:
				TOTAL += 1
